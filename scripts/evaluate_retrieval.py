...
def average_precision_at_k(
    ...
    AP@K = (1/|gold|) * sum_{i=1}^{K} P(i) * rel(i)
    ...
    return precision_sum / len(gold_pmids)
...
def compute_document_metrics(
    ...
    for k in K_VALUES:
        ...
        metrics[f"map_at_{k}"] = round(ap, 4)
...
def compute_snippet_metrics(
    ...
    for k in K_VALUES:
        ...
        for item in top_k_items:
            pmid = item.get("pmid", "")
            text = item.get("text", "")
            if pmid in {s["pmid"] for s in gold_snippets}:
                for snippet in gold_snippets:
...
def _build_summary(
    ...
    for k in K_VALUES:
        ...
        doc_agg[f"MAP@{k}"] = mean_metric(all_doc_metrics, f"map_at_{k}")
