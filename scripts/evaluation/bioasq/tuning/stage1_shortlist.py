"""
Stage 1 BioASQ generation tuning:
- Run ROUGE-SU4 only
- Use the first 20 validation questions
- Return the top-5 candidate configs
"""
import argparse
import sys
from pathlib import Path
from typing import List

# Configure project root
project_root = Path(__file__).resolve().parent.parent.parent.parent.parent
if str(project_root) not in sys.path:
    sys.path.append(str(project_root))

from config.logging_config import setup_logging
from config.settings import settings
from scripts.evaluation.shared import generation_bioasq_common as common
from scripts.evaluation.bioasq.tuning import tuning_common


VAL_PATH = settings.DATA_PATH / "val" / "val_bioasq.jsonl"


def run_stage1(limit: int = tuning_common.STAGE1_LIMIT) -> None:
    """Run Stage 1 tuning and save the top-5 shortlist."""
    output_root = tuning_common.stage1_output_root()
    output_root.mkdir(parents=True, exist_ok=True)

    leaderboard: List[dict] = []
    candidates = tuning_common.generate_stage1_candidates()

    for index, config in enumerate(candidates, 1):
        run_name = tuning_common.candidate_run_name(index, config)
        run_output_dir = output_root / run_name

        common.evaluate_split(
            data_path=VAL_PATH,
            output_dir=run_output_dir,
            split_name="validation",
            limit=limit,
            use_ragas=False,
            kg_top_k=config["kg_top_k"],
            kg_hop1_m=config["kg_hop1_m"],
            kg_hop2_n=config["kg_hop2_n"],
            kg_hop2_cap=config["kg_hop2_cap"],
            rerank_kg_top_n=config["rerank_kg_top_n"],
            generation_temperature=config["generation_temperature"],
            use_kg_merger=config["use_kg_merger"],
            use_head_tail_placement=config["use_head_tail_placement"],
            generation_max_tokens=config["generation_max_tokens"],
        )

        summary_path = run_output_dir / "summary.json"
        summary = tuning_common.load_summary(summary_path)
        score = tuning_common.stage1_primary_score(summary)
        leaderboard.append(
            {
                "rank_candidate": run_name,
                "score": score,
                "summary_path": str(summary_path),
                "config": summary["config"],
                "generation_metrics": summary["generation_metrics"],
            }
        )

    leaderboard = sorted(leaderboard, key=lambda item: item["score"], reverse=True)
    shortlist = tuning_common.shortlist_top_k(
        leaderboard,
        tuning_common.TOP_K_STAGE1_SHORTLIST,
    )

    tuning_common.save_json(leaderboard, output_root / "leaderboard.json")
    tuning_common.save_json(shortlist, output_root / "top5_candidates.json")


def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Stage 1 BioASQ generation tuning.")
    parser.add_argument("--limit", type=int, default=tuning_common.STAGE1_LIMIT)
    return parser


if __name__ == "__main__":
    setup_logging()
    args = build_arg_parser().parse_args()
    run_stage1(limit=args.limit)
